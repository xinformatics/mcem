...
class JointFeatureGenerator(nn.Module):
    ...
    def likelihood_distribution(self, past: th.Tensor):
        ...
        cov_noise = (
            th.eye(self.feature_size).unsqueeze(0).repeat(len(z), 1, 1) * 1e-2 ## old was 1e-5 also tried 1e-4, 1e-3, 1e-2, 1e-1 but fails at some step
        ).to(z.device)
    ...
    def forward_conditional(
        ...
        covariance_cond = self.clamp_eigenvalues(covariance_cond)
    ...
    def clamp_eigenvalues(self, matrix: th.Tensor, min_value: float = 1e-5) -> th.Tensor:
        """
        Projects 'matrix' onto the space of positive-definite matrices
        by clamping its eigenvalues to 'min_value'.
        'matrix' is of shape [batch_size, d, d].
        """
        # Symmetrize to avoid numerical issues
        sym_matrix = 0.5 * (matrix + matrix.transpose(-1, -2))

        # Eigen-decomposition
        e_vals, e_vecs = th.linalg.eigh(sym_matrix)  # shapes: [B, d], [B, d, d]

        # Clamp eigenvalues
        e_vals_clamped = e_vals.clamp(min=min_value)

        # Reconstruct
        clamped_matrix = (e_vecs * e_vals_clamped.unsqueeze(-2)) @ e_vecs.transpose(-1, -2)

        # Might need to symmetrize again due to floating errors
        clamped_matrix = 0.5 * (clamped_matrix + clamped_matrix.transpose(-1, -2))
