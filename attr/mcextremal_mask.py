...
from .models import MCExtremalMaskNet
from tint.models import MLP, RNN


class MCExtremalMask(PerturbationAttribution):
    """
    Extremal masks.
    ...
    def attribute(
        ...
        mask_net: MCExtremalMaskNet = None,
        batch_size: int = 32,
        ...
        if trainer is None:
            trainer = Trainer(max_epochs=500,enable_model_summary=False) #
        else:
        ...
        if mask_net is None:
            # print('Uni RNN used!')
            mask_net = MCExtremalMaskNet(forward_func=self.forward_func, 
                # model=nn.Sequential( RNN( input_size=231, rnn="gru", hidden_size=231, bidirectional=True,),MLP([2 * 231, 231]),)
                model=nn.Sequential( RNN( input_size=231, rnn="gru", hidden_size=231, bidirectional=False,),MLP([231, 231]),)
        ...
        if return_uncertainty:
            # Compute Monte Carlo dropout estimates.
            mask_net.net.train()  # Enable dropout.
            mc_masks = []
            for _ in range(num_mc_samples):
                mc_masks.append(mask_net.net.representation(apply_dropout=True))
            mc_masks = th.stack(mc_masks, dim=0)
            mask_mean = mc_masks.mean(dim=0)
            mask_variance = mc_masks.var(dim=0)
            attributions = (mask_mean, mask_variance)
        else:
            attributions = (mask_net.net.representation(),)

        if return_temporal_attributions:
            attributions = tuple(a.reshape((-1, data.shape[1]) + data.shape[1:]) for a in attributions)


        if return_uncertainty:
            # Directly return the tuple of (mask_mean, mask_variance)
            return attributions
        else:
